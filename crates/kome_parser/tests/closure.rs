...
fn parses_single_parameter_closure() {
    let expression = parse_expression("|value| value * 2").unwrap();
    ...
    let Pattern::Ident(parameter) = &closure.params[0] else {
    ...
    let Expression::Binary(body) = closure.body.as_ref() else {
...
fn parses_multiple_parameter_closure() {
    let expression = parse_expression("|left, right| left + right").unwrap();
    ...
    let Pattern::Ident(left) = &closure.params[0] else {
        panic!("expected left parameter");
    };

    let Pattern::Ident(right) = &closure.params[1] else {
...
fn parses_typed_closure_parameter() {
    let expression = parse_expression("|value: Number| value * 2").unwrap();
...
fn parses_closure_with_block_body() {
    ...
    let Expression::Block(body) = closure.body.as_ref() else {
...
fn parses_closure_as_call_argument() {
    let expression = parse_expression("items.map(|item| item.name)").unwrap();
...
fn parses_closure_as_top_level_initializer() {
    let module = parse("let double = |value| value * 2").unwrap();

    let Declaration::Let(binding) = &module.declarations[0] else {
        panic!("expected top-level let");
    };

    assert!(matches!(&binding.init, Some(Expression::Closure(_))));
}

#[test]
fn parses_nested_closure() {
    let expression = parse_expression("|value| |other| value + other").unwrap();

    let Expression::Closure(outer) = expression else {
        panic!("expected outer closure");
    };

    let Expression::Closure(inner) = outer.body.as_ref() else {
...
fn rejects_closure_without_closing_pipe() {
    let error = parse_expression("|value value * 2").unwrap_err();
...
fn rejects_trailing_parameter_comma() {
    let error = parse_expression("|left,| left").unwrap_err();
    ...
    assert_eq!(
        error.kind,
        ParseErrorKind::Expected {
            expected: "a closure parameter after `,`",
            found: TokenKind::Pipe,
        },
    );
}
...
fn logical_or_remains_binary_expression() {
    let expression = parse_expression("left || right").unwrap();
