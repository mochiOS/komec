...
fn parse_raw(value: &str) -> u64 {
    unsafe { __kome_number_parse(value.as_ptr(), value.len()) }
}
...
fn adds_numbers_through_abi() {
    ...
    let result = unsafe { __kome_number_add(left, right) };
...
fn subtracts_numbers_through_abi() {
    ...
    let result = unsafe { __kome_number_sub(left, right) };
...
fn multiplies_numbers_through_abi() {
    ...
    let result = unsafe { __kome_number_mul(left, right) };
...
fn compares_numbers_through_abi() {
    ...
    assert_eq!(unsafe { __kome_number_compare(smaller, larger) }, -1);

    assert_eq!(unsafe { __kome_number_compare(larger, smaller) }, 1);

    assert_eq!(unsafe { __kome_number_compare(smaller, smaller) }, 0);
